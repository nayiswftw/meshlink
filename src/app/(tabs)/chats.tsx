/**
 * Chats tab — Conversation list sorted by recent activity.
 */
...
function ConversationItem({ conv }: { conv: Conversation }) {
    const color = peerColor(conv.peerPublicKey || conv.peerId);
            ...
            {conversations.length === 0 ? (
                <View className="flex-1 items-center justify-center px-8">
                    ...
                    <Text className="text-[#7A7A7A] text-sm text-center max-w-xs">
                        Connect to a nearby peer from the Peers tab to start chatting
                    </Text>
                    <TouchableOpacity
                        onPress={() => router.push('/(tabs)/peers')}
                        className="bg-[#5C6B3C] flex-row items-center rounded-xl px-6 py-3.5 mt-8"
                        style={{
                            shadowColor: '#5C6B3C',
